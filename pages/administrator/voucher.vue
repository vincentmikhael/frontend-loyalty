    ...
    <div>
        <div class="container">
            <div class="d-flex mt-3 align-items-center justify-content-between">
                <p class="mb-0 pb-0">Voucher Management</p>
...
export default{
    ...
    methods: {
        ...
        async handleUpload(){
            if(Object.keys(this.formdata).length == 6){
                ...
                let res = await this.$api.post('voucher/add', formData)
        ...
        async editLevel(id){
            ...
            this.isOpen = true
            this.isEdit = id
            let res = await this.$api.get('voucher/get?id='+id)
            this.formdata = {
                id: res.data.data.id,
        ...
        async handleEdit(){
            ...
            let res = await this.$api.put('/voucher/edit/'+this.isEdit,fData)
        ...
        async handleDelete(id){
            let res = await this.$api.delete('/voucher/delete/'+id)
            this.$toast.success(res.data.message)
            await this.$fetch()
        }
    ...
    async fetch() {
        ...
        let res = await this.$api.get('voucher/get')
